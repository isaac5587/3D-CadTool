...
public class CadLine {
	...
	public boolean intersects(double xa, double ya, double xb, double yb) {
		...
		// then check to validate the yx of the two points are on opposite sides of the
		// line
	...
	public boolean intersects(CadBox box) {
		...
		if ((x0 < box.xMin && x1 < box.xMin) || (x0 > box.xMax && x1 > box.xMax) || (y0 < box.yMin && y1 < box.yMin)
		...
		return (intersects(box.xMin, box.yMax, box.xMax, box.yMax) || // top side of box
				...
				intersects(box.xMin, box.yMin, box.xMax, box.yMin)); // left side of box
	}
}
